<template>
  <section class="p-6 md:p-10" data-aos="fade-up">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
      <div v-for="(item, index) in menuItems" :key="index" :data-aos="'zoom-in'" :data-aos-delay="200 + index * 200">
        <MenuCard :title="item.title" :items="item.items" :image="item.image" />
      </div>
    </div>
  </section>
